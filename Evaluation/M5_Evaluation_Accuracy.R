...
for (sub_id in 1:length(sub_names)){
...
avg <- ddply(errors_total, .(level), colwise(sum))[,c(1,c(4:ncol(errors_total)))]
avg <- rbind(avg, colMeans(avg))

setwd(".../Accuracy Submissions/Errors")
if (benchmarks==1){
  ...
  write.csv(avg,"Accuracy_top50_overall.csv", row.names = F)
  write.csv(errors_total,"Accuracy_top50_results.csv", row.names = F)
  write.csv(evaluation_matrix_fh,"Accuracy_top50_results_fh.csv", row.names = F)
}
